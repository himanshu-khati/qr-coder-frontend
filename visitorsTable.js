...
import { getVisitorStats } from "./api/getVisitorStats.js";
...
const totalQrCodesElement = document.getElementById("totalQrCodes");
const checkedInVisitorsElement = document.getElementById("checkedInVisitors");
const pendingVisitorsElement = document.getElementById("pendingVisitors");
...
const fetchDashboardStats = async () => {
  try {
    const { success, data } = await getVisitorStats();
    if (!success) {
      throw new Error("Failed to load dashbiard data");
    }
    totalVisitorsElement.textContent = data.totalVisitors ?? "-";
    checkedInVisitorsElement.textContent = data.checkedInVisitors ?? "-";
    totalQrCodesElement.textContent = data.qrCodesGenerated ?? "-";
    pendingVisitorsElement.textContent = data.pendingVisitors ?? "-";
  } catch (error) {
    showToast(`Error fetching dashboard stats`, "danger");
  }
};
...
const fetchVisitors = async (page = 1) => {
  try {
    ...
    startEntry.textContent = (currentPage - 1) * pageSize + 1;
    endEntry.textContent = Math.min(
      ...
      <tr>
        <td colspan="8" class="text-center">
          <div class="d-flex flex-column align-items-center justify-content-center p-5">
            ...
            <div>
            <a href="visitor-create-single.php" class="btn btn-primary m-2 ">    
              Add Single Visitor
            </a>
            <a href="visitor-create-bulk.php" class="btn btn-primary m-2">   
             Bulk Add Visitors
            </a>
            </div>
          </div>
...
const renderPaginationControls = () => {
  ...
  prevButton.addEventListener("click", (e) => {
    e.preventDefault();
    if (currentPage > 1) fetchVisitors(currentPage - 1);
  });
  ...
  nextButton.addEventListener("click", (e) => {
    e.preventDefault();
    if (currentPage < totalPages) fetchVisitors(currentPage + 1);
  });
  ...
  sendEmailButtons.forEach((button) => {
    ...
    button.addEventListener("click", async (event) => {
      ...
      try {
        ...
        if (result.success) {
          showToast(result.message || "email send successfully", "success");
        }
      } catch (error) {
